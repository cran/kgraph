test_cosine_simi = function() {
  ...
  # vector to vector
  myres = kgraph:::cosine_simi(m_embeds[1, , drop = FALSE],
  ...
  myres = kgraph:::cosine_simi(m_embeds[1, , drop = FALSE], m_embeds[3:4, ])
  expect_equal(as.numeric(myres[1]), as.numeric(thebench[1]))

  myres = kgraph:::cosine_simi(m_embeds[2, , drop = FALSE], m_embeds[3:4, ])
